...
# Base AntEnv from Mujoco Gym.
class AntEnvRandom(MujocoEnv, utils.EzPickle):
    """
    ### Description
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def reset_model(self):
        ...
        # Random Initialization of main body
        init_qpos = self.init_qpos.copy()
        init_qpos[0], init_qpos[1] = 0, 0 
...
class CustomAntEnvNoEnergy(gym.Wrapper):
    def __init__(self, xml_file_path="/home/yamen/data/study/phd/code/MOP/ant.xml", **kwargs):
        
        self.env = AntEnvRandom(xml_file=xml_file_path, **kwargs)
        super(CustomAntEnvNoEnergy, self).__init__(self.env)
    ...
    def reset(self):
        observation, init_qpos = self.env.reset()[0]
        return observation, init_qpos
    
class CustomAntEnv(gym.Wrapper):
    def __init__(self, init_energy=200, xml_file_path="ant_energy.xml", **kwargs):
        
        self.env = AntEnvRandom(xml_file=xml_file_path, **kwargs)
        super(CustomAntEnv, self).__init__(self.env)
            
        self.gain = 25
        self.init_energy = 200 
        self.max_energy = 400
        self.energy = self.init_energy
        self.energy_sources = [[2, 2]]
        self.observation_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(34,))

         
    def step(self, action):
        observation, reward, done, _, info = self.env.step(action)
        self.energy -= 1 
        # add x_position and y_position to the observation
        x_pos = info.get('x_position', 0)
        y_pos = info.get('y_position', 0)

        state_pos = [self.energy]
        state_pos.append(x_pos)
        state_pos.append(y_pos)
        for s in self.energy_sources:
            state_pos.append(s[0] - x_pos)
            state_pos.append(s[1] - y_pos)
            state_pos.append(s[0])
            state_pos.append(s[1])
            

        extended_observation = np.append(list(observation), state_pos)
        observation = extended_observation
        
        for s in self.energy_sources:
            distance = np.sqrt((s[0] - x_pos)**2 + (s[1] - y_pos)**2)
    ...
    def reset(self):
        observation, init_qpos = self.env.reset()[0]
        self.energy = self.init_energy
        x_pos, y_pos = init_qpos[0], init_qpos[1]
        state_pos = [self.energy]
        state_pos.append(x_pos)
        state_pos.append(y_pos)
        for s in self.energy_sources:
        ...
        extended_observation = np.append(list(observation), state_pos)
        observation = extended_observation
        observation = (extended_observation, observation[1])
        return observation
