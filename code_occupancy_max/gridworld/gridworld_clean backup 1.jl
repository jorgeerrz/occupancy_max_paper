...
# ╔═╡ 9e10677f-49cc-4de3-9702-3e21fcd3c648
...
function emp_episode(s_0,u_0,env,pars_emp;n_steps = 10)
	...
	push!(us_out,u)
	for t in 1:n_steps
		actions,_ = adm_actions(s,u,env,pars_emp.constant_actions)
		#n_step_actions = Iterators.product([actions for i in 1:pars_emp.n]...)
		#N_n = length(actions)^pars_emp.n
		#emps = zeros(N_n)
...
#states_emp,us_emp, actions_emp,emp_emp = emp_episode([3,6],30,env_emp,p_emp,n_steps = 200)
...
# ╔═╡ 802f46a4-3668-43ef-a45b-27df61fc30a4
#=╠═╡
states_emp
  ╠═╡ =#
	...
	for x in 1:env.sizex
		for y in 1:env.sizey 
			...
			if length(ids) == 0
				...
				aux = actions.*probs
				for i in 1:length(aux)
					arrow_x[i] = aux[i][1]*0.6
					arrow_y[i] = aux[i][2]*0.6
				end
				arrow0!.(ones(Int64,length(aux))*x,ones(Int64,length(aux))*y,arrow_x,arrow_y,as = 0.3)
...
begin
	...
	#savefig("optimal_value_function_deterministic.png")
end
...
function many_episodes_histogram(s_0,u_0,h_value,q_value,ϵ,max_t,n_episodes,env::environment,params)
	...
	for j in 1:n_episodes
		...
		q_xpos,q_ypos,q_us,q_allx,q_ally,q_visited_s,q_visited_u,q_time = sample_trajectory_q(s_0,u_0,q_value,ϵ,max_t,env,false)
		rand_xpos,rand_ypos,rand_us,rand_allx,rand_ally,rand_visited_s,rand_visited_u,rand_time = sample_trajectory_random(s_0,u_0,max_t,env,false)
		push!(times_h,h_time)
		push!(times_q,q_time)
...
if movie
	Random.seed!(2)
	h_xpos_anim,h_ypos_anim,h_us_anim,h_allx_anim,h_ally_anim,h_urgency_anim = sample_trajectory(s_0,u_0,h_value,max_t_anim,env1,params)
...
begin
	...
	_,_,q_us,_,_,_,_,_ = sample_trajectory_q(s_0,u_0,q_value,ϵ,max_t,env1,false)
end
