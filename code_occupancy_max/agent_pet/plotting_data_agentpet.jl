...
begin
	theme(:default,titlefont = ("Computer Modern",16), legend_font_family = "Computer Modern", legend_font_pointsize = 14, guidefont = ("Computer Modern", 16), tickfont = ("Computer Modern", 14),colorbar_titlefont = ("Computer Modern",14))
...
function draw_arena(plt,N)
	...
	#Obstacle
	plot!(plt,[(N-2+offset,0+offset),(N-2+offset,N+offset)],label = false,color = :lightblue,lw = 4,ls = :dash)
	plot!(plt,[(N-3+offset,0+offset),(N-3+offset,N+offset)],label = false,color = :lightblue,lw = 4,ls = :dash)
	#plot!(plt,[(2+offset,4+offset),(2+offset,N-1+offset),(3+offset,N-1+offset),(3+offset,4+offset),(2+offset,4+offset)],label = false, color = :black,lw = 2)
		#Draw switch
	plot!(plt, [1],[1],st = :scatter, label = false,color = :lightblue, markershape = :rtriangle,markersize = 12)
end
...
begin
	lim_occs = (0,0.09)
	plot_occ_1 = plot(grid = false,minorgrid = false, ticks = false,title = "Pet of (A-S)-H agent")
	plot_occ_0 = plot(grid = false,minorgrid = false, ticks = false,title = "Pet of (A)-H agent")
	#Plot occupations
	...
	# 	end
	# end
	plot(plot_occ_0,plot_occ_1,layout = Plots.grid(1, 2, widths=[0.45,0.55]),size = (800,400),margin = 4Plots.mm,axis = false)
	#savefig("occupations_episode.pdf")
end
...
begin
	plot(collect(0:0.1:1),transpose(mean(open_rate,dims = 1)),yerror = transpose(std(open_rate,dims = 1))/sqrt(n_episodes),lw = 2,c = :black,markerstrokewidth = 2,label = false,ylabel = "Fraction of time fence open", xlabel = "\$\\beta\$",grid = false,margin = 3Plots.mm,size = (400,400))
