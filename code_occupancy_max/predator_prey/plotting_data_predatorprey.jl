...
begin
	arena = plot(ticks = false, size = (300,300),axis= false)
	plot!(arena, [2],[7],st = :scatter, label = false,color = :red, markershape = :dtriangle,markersize = 8)
	plot!(arena, [4],[7],st = :scatter, label = false,color = :gray, markershape = :utriangle,markersize = 8)
...
begin
	...
	for x in 1:N
		for y in 1:N		
			...
			for i in 1:9
				arrow0!(plot_occ_H,x,y,arrows1[i][1],arrows1[i][2],as = 0.2,lc = :white)
				arrow0!(plot_occ_R,x,y,arrows3[i][1],arrows3[i][2],as = 0.2,lc = :white)
			end
		end
	end
	plot(plot_occ_H,plot_occ_R,layout = Plots.grid(1, 2, widths=[0.45,0.55]),size = (800,400),margin = 4Plots.mm,axis = false)
...
begin
	...
	for i in 1:8
		...
		rotH = npzread("data_plotted/rot2co/rot2co_zformedcommonW_mouselab_bottleneck_fulltank N7F$(F0+i)beta2.0miop0nz0.0has0_steps5001")
		rotR = npzread("data_plotted/rot2co/rot2co_dp2_zformedcommonW_mouselab_bottleneck_fulltank N7F$(F0+i)beta2.0miop0nz$(noises[i])_steps5001")
		push!(rot_H,rotH)
		push!(rot_R,rotR)
	end
end
...
begin
	plot_rot = plot(ylabel = "Frac. of clockwise rotations",xlabel = "Food gain",grid = false,size = (500,350),legend_foreground_color = nothing,margin = 3Plots.mm)#,ylim = (0.4,1.0))
...
begin
	plot_life = plot(ylabel = "Survived time steps",xlabel = "Food gain",legend_foreground_color = nothing,size = (500,400),grid = false)
